...
export default function SaldosPage() {
  ...
  const handleExport = async (type: "excel" | "pdf") => {
    try {
        const response = await axios.post(`http://localhost:8000/export/${type}`, filteredData, {
            responseType: "blob", 
        });
  ...
  return (
    <div className="space-y-6 pb-20 p-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div>
           <h1 className="text-2xl font-bold text-[#183C30]">Saldos Consolidados</h1>
           <p className="text-gray-500 text-sm">Inventario en tiempo real multi-empresa</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <KpiCard title="Total Registros" value={filteredCount.toString()} />
               ...
               {filteredData.length > 0 ? (
                 filteredData.map((item, idx) => (
                   <tr key={idx} className="hover:bg-green-50/30 transition-colors group">
                     <td className="px-6 py-3 font-medium text-[#183C30]">{item.company_name}</td>
                 ...
                 <tr>
                    <td colSpan={5} className="px-6 py-20 text-center text-gray-400">
                       <div className="flex flex-col items-center justify-center space-y-3">
                         ...
                         <p>{isLoading ? "Sincronizando inventarios..." : "No se encontraron datos coincidentes."}</p>
                       </div>
                    </td>
                 </tr>
               )}
