...
export default function ComprasPage() {
    ...
    const exportarOrdenPDF = (orden: OrdenCompra, tercero: Tercero) => {
        ...
        doc.text("FIRMA SOLICITANTE", 14, finalY + 5);

        // Columna Derecha: Autorización
        doc.text("_________________________", 120, finalY);
        doc.text("FIRMA AUTORIZACIÓN", 120, finalY + 5);
                        ...
                        {viewingOrden && (
                            <div className="bg-white rounded-lg overflow-hidden flex flex-col h-full">
                                    ...
                                    <div className="grid grid-cols-2 gap-12">
                                        ...
                                        <div className="space-y-5">
                                            ...
                                            <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 space-y-5 shadow-sm">
                                                <div className="grid grid-cols-3 gap-2">
                                                    ...
                                                    <span className="col-span-2 font-black text-slate-800 text-lg leading-tight">{terceros.find(t => t.id === viewingOrden.terceroId)?.nombre || 'Desconocido'}</span>
                                                </div>
                                                <div className="grid grid-cols-3 gap-2">
                                                    <span className="text-slate-400 text-[10px] font-black uppercase tracking-widest">NIT / NIT</span>
                                                    <span className="col-span-2 font-mono font-bold text-slate-600 bg-white px-3 py-1 rounded border inline-block text-sm shadow-sm">{terceros.find(t => t.id === viewingOrden.terceroId)?.nit || 'N/A'}</span>
                                                </div>
                                                <div className="grid grid-cols-3 gap-2 border-t pt-4 border-slate-200/50">
                                                    ...
                                                    <div className="col-span-2 flex flex-col">
                                                        <span className="font-black text-slate-700">{terceros.find(t => t.id === viewingOrden.terceroId)?.personaContacto || 'N/A'}</span>
                                                        <span className="text-sm font-bold text-slate-500">{terceros.find(t => t.id === viewingOrden.terceroId)?.numeroContacto || 'N/A'}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-5">
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                ...
                                                {viewingOrden.entregasParciales && (
                                                    <div className="col-span-2 bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100 space-y-1">
                                                        ...
                                                        <p className="text-sm text-emerald-900 font-bold">{viewingOrden.entregasParciales}</p>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                ...
                                <div className="p-10 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
                                    <Button variant="ghost" onClick={() => setIsViewDialogOpen(false)} className="px-10 font-black text-slate-400 hover:text-slate-600 h-14 rounded-2xl uppercase tracking-widest text-xs hover:bg-slate-100">
                                        Cerrar Vista
                                    </Button>
                                    <Button onClick={() => {
