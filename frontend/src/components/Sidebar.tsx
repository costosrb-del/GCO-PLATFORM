  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 p-2 space-y-2 overflow-y-auto overflow-x-hidden">
              ...
              {(isInventoryOpen || isCollapsed) && (
                ...
                >
                  <div className={`${!isCollapsed ? "ml-4 pl-4 border-l border-[#2A5E4D]" : "flex flex-col items-center"} space-y-1 pt-1`}>
                    ...
                    <Link
                      href="/dashboard/cierre-actas"
                      className={`block px-3 py-2 rounded-lg text-sm transition-all ${pathname === "/dashboard/cierre-actas"
                        ? "bg-white text-[#183C30] font-medium shadow-md"
                        : "text-gray-400 hover:text-white hover:bg-white/5"
                        } ${isCollapsed ? "text-center text-[10px]" : ""}`}
                      title={isCollapsed ? "Actas de Cierre" : ""}
                    >
                      {isCollapsed ? "Actas" : "Actas de Cierre"}
                    </Link>
        ...
        {/* Tasks (New Top Level) */}
        {(role === "admin" || role === "asesora" || role === "viewer") && (
          <Link
            href="/dashboard/tareas"
            className={`flex items-center space-x-3 px-3 py-3 rounded-xl transition-all group ${pathname === "/dashboard/tareas"
              ? "bg-white/10 text-white font-medium"
              : "text-gray-300 hover:bg-white/5 hover:text-white"
              } ${isCollapsed ? "justify-center" : ""}`}
